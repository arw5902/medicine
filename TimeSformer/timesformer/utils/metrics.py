...
def topks_correct(preds, labels, ks):
    ...
    logger.info("In topks_correct...preds is {}, ks is {}".format(preds, ks))
    # Find the top max_k predictions for each sample
    _top_max_k_vals, top_max_k_inds = torch.topk(
        preds, max(ks), dim=1, largest=True, sorted=True
    )
    logger.info("In topks_correct..._top_max_k_vals is {}, top_max_k_inds is {}".format(_top_max_k_vals, top_max_k_inds))
    # (batch_size, max_k) -> (max_k, batch_size).
    top_max_k_inds = top_max_k_inds.t()
    logger.info("In topks_correct...top_max_k_inds becomes {}".format(top_max_k_inds))
    # (batch_size, ) -> (max_k, batch_size).
    rep_max_k_labels = labels.view(1, -1).expand_as(top_max_k_inds)
    logger.info("In topks_correct...rep_max_k_labels is {}".format(rep_max_k_labels))
    # (i, j) = 1 if top i-th prediction for the j-th sample is correct.
    top_max_k_correct = top_max_k_inds.eq(rep_max_k_labels)
    logger.info("In topks_correct...top_max_k_correct is {}".format(top_max_k_correct))
    # Compute the number of topk correct predictions for each k.
    topks_correct = [top_max_k_correct[:k, :].float().sum() for k in ks]
    logger.info("In topks_correct...topks_correct is {}".format(topks_correct))
